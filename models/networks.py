...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02,
    ...
    Parameters:
        ...
        netG (str) -- the architecture's name: resnet_9blocks | resnet_6blocks | relational_resnet_9blocks | relational_resnet_6blocks | unet_256 | unet_128
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02,
    ...
    Our current implementation provides three types of discriminators:
        ...
        [relational]: With this mode, you can specify the number of conv layers in the discriminator
        with the parameter <n_layers_D> (default=3 as used in [basic] (PatchGAN).). This layer also adds a relational
        layer for each conv layer in the discriminator.
...
class RelationalResnetGenerator(nn.Module):
    """Resnet-based generator that consists of Resnet blocks between a few downsampling/upsampling operations.
...
class RelationalLayer(nn.Module):
    ...
    def forward(self, cnn_feats):
        ...
        x = cnn_feats
        #print(x.size())
        x = F.adaptive_max_pool2d(x, (self.filter_size, self.filter_size))
        ...
        # x_flat = (2 x 256 x 24)
        # add coordinates
        # ensure both tensor sizes match for mult-gpu case. Allows for difference in init and forward states
        #print(x_flat.size(), self.coord_tensor.size())
        x_flat = torch.cat([x_flat, self.coord_tensor if self.coord_tensor.size()[0] == mb else torch.FloatTensor(mb, 16, 2).cuda()], 2)  # (2 * 16 * 26)
        #print('A', x_flat.size())

        # cast all pairs again48st each other
        x_i = torch.unsqueeze(x_flat, 1)  # (2 * 1 * 16 * 26)
        #print('B', x_i.size())
        x_i = x_i.repeat(1, d*d, 1, 1)  # (2 * 16 * 16 * 26)
        #print('C', x_i.size())

        x_j = torch.unsqueeze(x_flat, 2)  # (2 * 16 * 1 * 26)
        #print('D', x_j.size())
        x_j = x_j.repeat(1, 1, d*d, 1)  # (2 * 16 * 16 * 26)
        #print('E', x_j.size())

        # concatenate all together
        #print(x_i.shape, x_j.shape)
        x_j = x_j.permute(0, 2, 1, 3)
        #print(x_i.shape, x_j.shape)
...
class RelationalNLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, batch_size=1, gpu_ids=[], device=None,
        ...
        self.relational_layer = RelationalLayer(cnn_feats, batch_size=int(batch_size / len(gpu_ids)))
